...
log using "D:\1. M2 Development Economics\0. Thesis\Thesis\Logs\4 robustness", replace
...
preserve
collapse (mean) dln_mu Z_f1 Z_f2 Z_f3 Z_input age leverage lnSize ls_pre_filled post2016, by(firm_id isic4 year)
xtset firm_id year
...
esttab rob12_base rob12_sq rob12_ctr1 rob12_ctr2 rob12_ctr3 ///
    ...
    using "$folder\Tables\rob12_fe.tex", ///
...
save "rotemberg_sector_weights.dta", replace

restore

**** (2) Create exclusion lists for the most influential sectors

use "rotemberg_sector_weights.dta", clear
gsort -abs_alpha
...
save "$folder\Data\rotemberg_drop_lists.dta", replace

**** (3) Merge drop lists back to main data
use "$folder\Data\data_ready.dta", clear

merge m:1 isic4 using "$folder\Data\rotemberg_drop_lists.dta", nogen
...
*----------------------------
esttab est13_base est13_top5 est13_top10 est13_top10pc using "$folder\Tables\rob13_rotemberg.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
...
* Panel B: first stage
*----------------------------
esttab fs_change_IP fs5_change_IP fs10_change_IP fs10pc_change_IP using "$folder\Tables\rob13_rotemberg.tex", ///
...
filefilter "$folder\Tables\rob13_rotemberg.tex" "$folder\Tables\rob13_rotemberg_tmp.tex", from("\hline\hline") to("\hline"), replace
...
tempname memhold
postfile `memhold' alpha ssr N using "$folder\Data\kmt_grid_results.dta", replace
...
use "$folder\Data\kmt_grid_results.dta", clear
sort ssr
...
esttab rob21_iv1 rob21_iv2 using "$folder\Tables\rob21_kmt_iv.tex", replace ///
...
esttab rob22_base rob22_1 rob22_2 using "$folder\rob22_trimming.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
    keep(change_IP Z_input) ///
    stats(N, labels("Observations")) ///
...
esttab rob3_bal8 rob3_bal7 rob3_bal6 rob3_bal5 rob3_bal4 rob3_bal3 rob3_nomicro rob3_top1 using "$folder\rob3_composition.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
...
graph export "$folder\Figs\rob41_decquin.png", replace width(2200)
...
graph export "$folder\Figs\rob42_rank.png", replace width(2000)
restore
...
graph export "$folder\Figs\rob43_thin.png", replace width(2200)
