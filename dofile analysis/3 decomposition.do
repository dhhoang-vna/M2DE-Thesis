...
*--- export LaTeX table
esttab within between using "$folder\Tables\decomposition.tex", ///
    replace ///
...
*    100% sure it equals Δ(mu^{-1}_{jt}) computed from the same aggregation.
*
*    Here we compute sector inverse markup as a (market-share) weighted mean:
*       inv_mu_jt = Σ_i s_it * inv_mu_it
...
save "$folder\Data\balanced_sample.dta", replace
...
isid firm_id year
xtset firm_id year
merge m:1 isic4 year using "$folder\Data\balanced_sample.dta"
