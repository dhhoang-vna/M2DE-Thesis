...
log using "D:\1. M2 Development Economics\0. Thesis\Thesis\Logs\5_mechanism", replace
...
esttab mec11_mu using "$folder\Tables\mec11_mu.tex", ///
...
esttab q5 q10 q15 q20 q25 q30 q35 q40 q45 q50 q55 q60 q65 q70 q75 q80 q85 q90 q95 ///
    using "$folder\Tables\mec12_givq.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
...
save "$folder\Data\IV\H_j_mec2.dta", replace
...
merge m:1 industry_j using "$folder\Data\IV\H_j_mec2.dta", keep(match master) nogen
save "$folder\Data\IV\output_IV_H.dta", replace

use "$folder\Data\data_ready.dta", clear
capture drop _merge

merge m:1 isic4 year using "$folder\Data\IV\output_IV_H.dta"

save "$folder\Data\data_ready_H.dta", replace


*-------------------
* 2.2. Regressions 
*-------------------
use "$folder\Data\data_ready_H.dta", clear
...
* Table
esttab mec21 mec22 mec31 mec32 mec33 mec34 mec35 mec36 using "$folder\Tables\mec4.tex", replace se star(* 0.10 ** 0.05 *** 0.01) keep(change_IP Z_input changeIP_H Z_input_H) stats(N kpF, labels("Observations" "KP rk Wald F")) ///
...
* Table
*---------
esttab mec31_ip mec31_input mec32_ip mec32_input mec33_ip mec33_input using "$folder\Tables\mec3.tex", replace se star(* 0.10 ** 0.05 *** 0.01) keep(ip_size ip_large ip_cr4 input_size input_large input_cr4) stats(N F_first, labels("Observations" "F-stat")) ///
...
* Table
*---------
esttab mec41 mec42 mec43 using "$folder\Tables\mec4.tex", replace se star(* 0.10 ** 0.05 *** 0.01) keep(change_IP Z_input $X_lag) stats(N kpF, labels("Observations" "KP rk Wald F")) ///
