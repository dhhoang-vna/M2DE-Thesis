...
log using "$LOGS\2 selection_correction.log", replace
...
** markup and market share comovement? no
gen ln_share = ln(share_sales)
bysort isic4 year: gen dln_share = ln_share - ln_share[_n-1]
 
reghdfe dln_mu ln_share $X_lag if inrange(year, 2011, 2019), absorb(isic4 year firm_id) vce(cluster firm_id)
...
capture noisily estat endogenous llnsize lleverage lliquidity l_age l_exporter
...
esttab PERSIST EXIT_L EXIT_Q MU MARKOV using "$OUTPUT_TABLES/proxy_validation_table.tex", replace ///
...
twoway ///
    ...
    ytitle("Coefficient on import penetration", size(medsmall)) ///
...
graph export "$OUTPUT_FIGURES/iv_quantile_journal.png", width(2400) replace
...
* IP*CR10
*==========
capture confirm variable top10_share
if _rc gen top10_share = share_sales if rank_sales<=10 & dom_pos
capture confirm variable CR10_dom
if _rc bys isic4 year: egen CR10_dom = total(top10_share)

summ CR10_dom if inrange(year,2011,2019), meanonly
capture drop CR10_c
gen CR10_c = CR10_dom - r(mean)
...
foreach p of local percentiles {
    ...
    ivreghdfe q`p' ///
        (c.change_IP c.change_IP#c.CR10_dom = c.output_IV c.output_IV#c.CR10_dom) ///
        Z_input $n_poly $X_lag ///
        c.ls_pre_filled##i.post2016 i.year ///
        if inrange(year, 2011, 2019), ///
        absorb(isic4) vce(cluster isic4) partial(i.year)

    matrix results[`row',1] = _b[c.change_IP#c.CR10_dom]
    matrix results[`row',2] = _se[c.change_IP#c.CR10_dom]
    matrix results[`row',3] = e(N)
    matrix results[`row',4] = e(r2)
...
graph export "$OUTPUT_FIGURES/iv_quantile_cr10_journal.png", width(2400) replace
...
graph export "$OUTPUT_FIGURES/iv_quantile_cr4_journal.png", width(2400) replace
...
esttab exit mu_base mu_full mu_poly using "$OUTPUT_TABLES/prelim_1.tex", ///
    replace ///
