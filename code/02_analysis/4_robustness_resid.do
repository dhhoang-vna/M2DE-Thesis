...
log using "$LOGS\4_robustness_resid.log", replace
...
esttab fs_main fs_resid using "$OUTPUT_TABLES\rob_resid_firststage.tex", ///
...
esttab mu_main mu_resid using "$OUTPUT_TABLES\rob_resid_markup_main.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
...
esttab rr12_base rr12_sq rr12_ctr1 rr12_ctr2 rr12_ctr3 ///
    ...
    using "$OUTPUT_TABLES\rob_resid_fe.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
...
esttab aux_mu_main aux_mu_resid aux_rev_main aux_rev_resid ///
    ...
    using "$OUTPUT_TABLES\rob_resid_aux.tex", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
    keep(change_IP Z_input) ///
    order(change_IP Z_input) ///
...
sort outcome
export delimited using "$OUTPUT_TABLES\rob_resid_signcheck.csv", replace
save "$DATA_DERIVED\rob_resid_signcheck.dta", replace
list, noobs clean
restore
