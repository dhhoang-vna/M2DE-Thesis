...
log using "$LOGS\1 orbis_data.log", replace
...
save "$DATA_DERIVED\Orbis\TR_industry_classifications.dta", replace
...
save "$DATA_DERIVED\Orbis\industry_manuf_clean.dta", replace

** 2. Clean the main financial dataset and create the ready-to-use dataset

import delimited "$DATA_RAW\Orbis\TR_global_financials_and_ratios_eur.csv", clear
save "$DATA_DERIVED\Orbis\TR_global_financials_and_ratios_eur.dta", replace

* Merge the cleaned industry file
merge m:1 bvd_id_number using "$DATA_DERIVED\Orbis\industry_manuf_clean.dta"
...
save "$DATA_DERIVED\Orbis\orbis.dta", replace 

*-----------------------------------------
* Sectoral PPI (3-digit) as a deflator
*-----------------------------------------

import delimited "$DATA_RAW\TR_PPI.csv", delimiter("|") clear
...
save "$DATA_DERIVED\ppi_23.dta", replace

* Make the 3-digit PPI dataset
use "$DATA_DERIVED\ppi_23.dta", clear
...
save "$DATA_DERIVED\ppi_3.dta", replace
...
save "$DATA_DERIVED\ppi_2.dta", replace

* Merge PPI with Orbis dataset
use "$DATA_DERIVED\ORBIS\orbis.dta", clear
...
merge m:1 year nace3 using "$DATA_DERIVED\ppi_3.dta"
...
merge m:1 year nace2 using "$DATA_DERIVED\ppi_2.dta"
rename ppi ppi_2
