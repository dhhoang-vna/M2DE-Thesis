...
log using "$LOGS\6 labor_intensity.log", replace
...
** 4-digit
import excel "$DATA_RAW\personnel costs.xls", sheet("Sheet0") clear
...
save "$DATA_DERIVED\TurkStat\VA_cost_4.dta", replace
...
save "$DATA_DERIVED\TurkStat\VA_cost_3.dta", replace
...
save "$DATA_DERIVED\TurkStat\VA_costs_2.dta", replace
...
import excel "$DATA_RAW\VA factor cost.xls", sheet("Sheet0") clear
...
save "$DATA_DERIVED\TurkStat\personnel_costs_4.dta", replace
...
save "$DATA_DERIVED\TurkStat\personnel_cost_2.dta", replace
...
use "$DATA_DERIVED\TurkStat\personnel_costs_4.dta", clear
collapse (sum) personnel_cost, by(nace4 year)

count if missing(nace4) | missing(year)
drop if missing(nace4) | missing(year)
isid nace4 year
save "$DATA_DERIVED\TurkStat\personnel_costs_4_unique.dta", replace

use "$DATA_DERIVED\TurkStat\VA_cost_4.dta", clear
...
merge m:1 nace4 year using "$DATA_DERIVED\TurkStat\personnel_costs_4_unique.dta"
...
merge m:1 nace2 using "$DATA_DERIVED\Concordance\isic4_nace2.dta"
...
save "$DATA_DERIVED\TurkStat\labor_share_4.dta", replace
...
merge m:1 year using "$DATA_DERIVED\TurkStat\VA_costs_2.dta"

gen labor_share_tr = personnel_cost / VA_cost if personnel_cost>0 & VA_cost>0

drop _merge

save "$DATA_DERIVED\TurkStat\labor_share_2.dta", replace
...
*-------------------------------
import excel "$DATA_RAW\WIOD\Socio_Economic_Accounts.xlsx", sheet("DATA") firstrow clear
...
save "$DATA_DERIVED\ORBIS\labor_share_pre_wiod.dta", replace
